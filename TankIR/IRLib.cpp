...
IRrecvPCI::IRrecvPCI(unsigned char inum) 
{
    ...
    intrnum=inum;       // Sets the interrupt number (not the pin number). From IR_RECEIVE_INT_NUM in Settings.h. Should be 0 for Arduino External Interrupt 0
    ...
    // There are two bits in the control register for each of the 2 interrupts. We are interested in external interrupt 0
    // so we are going to be setting Interrupt Sense Control bits 0-0 and 0-1 (ISC00, ISC01). These bits determine
    ...
    // What this line does is basically clear the two ISC0 bits (not 1, ANDed with the two bits)
        EICRA = (EICRA & ~((1 << ISC00) | (1 << ISC01)));
    // Now we set ISC00 which will set it to interrupt on change
        EICRA |= (1 << ISC00);
    ...
    // EIMSK &= ~(1 << INT0);   // AND-NOT, instead of OR 
}
...
void IRrecvPCI::resume(void) 
{
    ...
    EIFR |= (1 << INTF0);
    // we set the appropriate bit in the External Interrupt Mask Register (EIMSK). We are turning on external interrupt 0
    // so we SET bit 0 by ORing a 1 shifted over to the correct bit. 
    EIMSK |= (1 << INT0);
}
...
// See Settings.h under Timer 2 for defines related to IR sending

volatile ir_send_params_t IR_SendParams;
                                
IRsendBase::IRsendBase () 
{
    // IRsendBase uses Timer 1 Compare B interrupt. Timer 1 is setup in the main sketch, using a macro defined in Settings.h
